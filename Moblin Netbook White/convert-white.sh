...
# This script requires ImageMagick and Inkscape to be installed.
# Run in the "Moblin Netbook White" directory.
...
cp -R ../Moblin\ Netbook/scalable . 

echo ""
echo "Replacing black with white in source files:"
echo ""
...
for category in `ls scalable/`; do
  ...
  echo -n "in folder \"$category\""
    for svg in *.svg; do
      cp $svg b-$svg
      cat b-$svg | sed s/#000000/#ffffff/g > $svg
      rm b-$svg
      echo -n "."
    done
  cd ../../
